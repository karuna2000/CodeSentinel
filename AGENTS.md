...
- **Chat answers are cached in Redis when `REDIS_URL` is set.** Key `codesintler:chat:{repo_id}:{commit_sha}:{query_hash}` (7-day TTL) — the commit SHA scopes entries to the index they were grounded in, so re-indexes auto-invalidate. Only served + grounded answers are stored; hits skip the LLM (no usage recorded) and emit `chat_cache_hit`, stores/skips emit `chat_cache_store[_failed]`/`chat_cache_skip`. Without `REDIS_URL` every question is a miss. Rate limiting is a separate client (@upstash/redis, REST) — a stock Redis container cannot serve it.
